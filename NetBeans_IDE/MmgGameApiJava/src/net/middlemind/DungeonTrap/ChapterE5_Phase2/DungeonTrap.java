package net.middlemind.DungeonTrap.ChapterE5_Phase2;
 ...
 */
public final class DungeonTrap {
    ...
    public static final void main(String[] args) {
        ...
        try {
            if (DungeonTrap.ENGINE_CONFIG_FILE != null && DungeonTrap.ENGINE_CONFIG_FILE.equals("") == false) {
                GameSettingsImporter dci = new GameSettingsImporter();
                boolean r = dci.ImportGameSettings(DungeonTrap.ENGINE_CONFIG_FILE);
                    ...
                    for (int i = 0; i < len; i++) {
                        try {
                            ...
                            if (ent.from != null && ent.from.equals("GameSettings") == true) {
                                ...
                                f = DungeonTrap.class.getField(ent.key);
                                if (f != null) {
                                    System.out.println("Importing " + ent.from + " field: " + ent.key + " with value: " + ent.val + " with type: " + ent.type + " from: " + ent.from);
                                    SetField(ent, f);
                                }                            
        ...
        mf = new MainFrame(DungeonTrap.WIN_WIDTH, DungeonTrap.WIN_HEIGHT, DungeonTrap.PANEL_WIDTH, DungeonTrap.PANEL_HEIGHT, DungeonTrap.GAME_WIDTH, DungeonTrap.GAME_HEIGHT);
        pnlGame = new GamePanel(mf, DungeonTrap.PANEL_WIDTH, DungeonTrap.PANEL_HEIGHT, (DungeonTrap.WIN_WIDTH - DungeonTrap.PANEL_WIDTH) / 2, (DungeonTrap.WIN_HEIGHT - DungeonTrap.PANEL_HEIGHT) / 2, DungeonTrap.GAME_WIDTH, DungeonTrap.GAME_HEIGHT);
        mf.SetGamePanel(pnlGame);
        mf.InitComponents();
        fr = new RunFrameRate(mf, FPS);

        mf.setSize(DungeonTrap.WIN_WIDTH, DungeonTrap.WIN_HEIGHT);
