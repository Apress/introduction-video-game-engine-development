package net.middlemind.MmgGameApiJava.EmptyProject;
 ...
 */
public final class MmgEmptyProject {
    ...
    public static final void main(String[] args) {
        ...
        try {
            if (MmgEmptyProject.ENGINE_CONFIG_FILE != null && MmgEmptyProject.ENGINE_CONFIG_FILE.equals("") == false) {
                GameSettingsImporter dci = new GameSettingsImporter();
                boolean r = dci.ImportGameSettings(MmgEmptyProject.ENGINE_CONFIG_FILE);
                    ...
                    for (int i = 0; i < len; i++) {
                        try {
                            ...
                            if (ent.from != null && ent.from.equals("GameSettings") == true) {
                                ...
                                f = MmgEmptyProject.class.getField(ent.key);
                                if (f != null) {
                                    System.out.println("Importing " + ent.from + " field: " + ent.key + " with value: " + ent.val + " with type: " + ent.type + " from: " + ent.from);
                                    SetField(ent, f);
                                }             
        ...
        mf = new MainFrame(MmgEmptyProject.WIN_WIDTH, MmgEmptyProject.WIN_HEIGHT, MmgEmptyProject.PANEL_WIDTH, MmgEmptyProject.PANEL_HEIGHT, MmgEmptyProject.GAME_WIDTH, MmgEmptyProject.GAME_HEIGHT);
        pnlGame = new GamePanel(mf, MmgEmptyProject.PANEL_WIDTH, MmgEmptyProject.PANEL_HEIGHT, (MmgEmptyProject.WIN_WIDTH - MmgEmptyProject.PANEL_WIDTH) / 2, (MmgEmptyProject.WIN_HEIGHT - MmgEmptyProject.PANEL_HEIGHT) / 2, MmgEmptyProject.GAME_WIDTH, MmgEmptyProject.GAME_HEIGHT);
        mf.SetGamePanel(pnlGame);
        mf.InitComponents();
        fr = new RunFrameRate(mf, FPS);

        mf.setSize(MmgEmptyProject.WIN_WIDTH, MmgEmptyProject.WIN_HEIGHT);
